...
class RSF(nn.Module):
    def __init__(self,
        ...
        self.gru = nn.RNN(input_size=data_dim,
...
def train_model(X_train:torch.Tensor,
    ...
    torch.save(model.state_dict(), f"./models/RSF-{hidden_size}-hidden-{data_dim}-input-{num_layers}-layer.pth")
    return model
...
def load_model(data_dim:int=526,
    ...
    '''
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = RSF(data_dim=data_dim,
                hidden_dim=hidden_dim,
                num_layers=num_layers,
                )
    model.load_state_dict(torch.load(f"./models/RSF-{hidden_dim}-hidden-{data_dim}-input-{num_layers}-layer.pth", map_location=device))
    print(f"RSF model with input_dim={data_dim}, hidden_dim={hidden_dim}, num_layers={num_layers} has been loaded.")
    return model
